...
library(latex2exp)
library(ggplot2)
load("resultsSection4_3.rda")

###################################################
resultsCovidMethodComparison <- na.omit(resultsCovidMethodComparison)
resultsCovidMethodComparison$case <- as.factor(paste0("case-",(resultsCovidMethodComparison$rho==1)+1))
...
#png("simu4_3Comparison.png", width = 1000, height = 600)
figure7 <- ggplot(resultsCovidMethodComparison, aes(x=size, y=value))+
  scale_fill_grey(start = 0.4, end = 0.8) +
  geom_boxplot(aes(fill=basis))  +
  facet_grid(model+case ~ bdelta + covariateeffect,scale="free")+
  labs(fill = "Method") +
...
#
# ###################################################
resultsCoviddata$case <- as.factor(paste0("case-",(resultsCoviddata$rho==1)+1))
...
resultsCoviddata$model <- as.factor(paste0("v^2= ", resultsCoviddata$model))
...
#png("simu4_3Covid.png", width = 1000, height = 600)
figure8 <- ggplot(resultsCoviddata, aes(x=size, y=value))+
  scale_fill_grey(start = 0.4, end = 0.8) +
  geom_boxplot(aes(fill=basis))  +
  facet_grid(model +case ~ bdelta + covariateeffect,scale="free")+
  labs(fill = "Method") +
